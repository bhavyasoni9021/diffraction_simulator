...
def multi_wavelength_intensity(theta, wavelengths, slit_width, slit_distance):
    total_intensity = np.zeros_like(theta)

    for wl in wavelengths:
        total_intensity += double_slit_intensity(
            theta, wl, slit_width, slit_distance
        )

    return total_intensity / len(wavelengths)

def estimate_slit_distance(wavelength, screen_distance, fringe_spacing):
    """
    Inverse problem:
    d = λL / y
    """
    ...
    return (wavelength * screen_distance) / fringe_spacing